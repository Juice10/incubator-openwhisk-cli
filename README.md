...
clean build of wsk. To that end, it's now necessary to use [https://github.com/tools/godep](https://github.com/tools/godep) to populate the
`vendors` folder using the versions selected in the `Godeps.json`:

```sh
$ go get github.com/tools/godep # Install the godep tool.
$ godep restore                 # Download and install packages with specified dependencies.
```
